  ...
  <div class="h-screen flex flex-col bg-white dark:bg-gray-900">
    <header class="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
      ...
      <div class="flex items-center gap-2">
        <Button :icon="appStore.themeMode === 'light' ? 'pi pi-moon' : 'pi pi-sun'"
          severity="secondary" size="small" text rounded @click="appStore.toggleTheme" />
    ...
    <div ref="shellBody" class="flex-1 flex overflow-hidden" :class="{ 'select-none': resizing }">
      <aside class="overflow-y-auto bg-gray-50 dark:bg-gray-800/50 shrink-0"
        :style="{ width: leftPanelWidth, minWidth: '550px', maxWidth: '36rem' }">
        <LeftPanel />
      </aside>
      <div class="w-1 cursor-col-resize shrink-0 bg-transparent hover:bg-blue-500 active:bg-blue-500 transition-colors"
        @mousedown.prevent="startResize('left', $event)" />
      <main class="flex-1 overflow-auto bg-gray-100 dark:bg-gray-800/30">
        <div class="flex flex-col items-center gap-4 py-4 min-h-full">
          <SelectButton v-model="appStore.deviceMode" :options="devices"
            option-value="mode" option-label="label" size="small" />
          <div :style="{ width: appStore.deviceWidths[appStore.deviceMode] + 'px',
            minWidth: appStore.deviceWidths[appStore.deviceMode] + 'px' }"
